...
import club.someoneice.makpiraaqarvik.event.PineappleConfigEvent;
...
public class ConfigBean {
    public static final Map<String, IPineappleConfig> CONFIG_MAP = Maps.newHashMap();
    public static final JSON JSON5_BEAN = JSON.json5;
    ...
    public ConfigBean(String fileName) {
        ...
        MapNode nodeBase1;
        File configFile = new File(System.getProperty("user.dir") + "config", fileName + ".json5");
    ...
    private <T> T getBean(String key, T defValue) {
        ...
        value = NeoForge.EVENT_BUS.post(new PineappleConfigEvent<T>(configName, key, value, null)).getValue();
    ...
    private  <T> T getBeanWithPackage(String key, T defValue, String packName) {
        ...
        value = NeoForge.EVENT_BUS.post(new PineappleConfigEvent<T>(configName, key, value, packName)).getValue();
